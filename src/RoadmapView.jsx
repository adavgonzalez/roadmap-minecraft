        ...
        <div style={{ padding:'11px 13px', display:'flex', gap:0, alignItems:'stretch' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
              marginTop:(step.badges?.length>0||hasNotes)?8:0, flexWrap:'wrap', gap:5 }}>
              ...
              {hasNotes && (
                <button onClick={() => setNotesOpen(o => !o)}
                  style={{ background:'none', border:`1px solid ${notesOpen?'#3b82f6':'#1c2b3f'}`,
                    ...
                    color:notesOpen?'#60a5fa':C.muted, transition:'all 0.15s', marginLeft:'auto' }}
...
function PhaseSection({ phase, dragProps, isDragging,
  ...
  function handleStepDragEnd({ active, over }) {
    ...
    db.steps.reorder(reordered)   // persist in background
  }
  ...
  return (
    <>
      <div style={{ background:C.panel, border:`1px solid ${isDragging?'#3b82f640':C.border}`,
        ...
        {/* Steps with their own DndContext */}
        {!collapsed && (
...
export default function RoadmapView({ phases, projectId, onPhasesChange }) {
  ...
  function handlePhaseDragEnd({ active, over }) {
    ...
    db.phases.reorder(reordered)  // persist in background
  }
